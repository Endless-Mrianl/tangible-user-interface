...
SERIAL_PORT = "/dev/ttyUSB0"  # ESP32 port
BAUD_RATE = 115200
    ...
    def start_video_processing(self):
        ...
        if self.use_arducam:
            ...
            try:
                ...
                # Open camera with CSI connection (same as preview_depth.py)
                ...
                # Set range (MAX_DISTANCE - 2000 or 4000)
                MAX_DISTANCE = 4000
            ...
            while True:
                if self.use_arducam:
                    ...
                    try:
                        ...
                        # Check if frame is DepthData type (as in preview_depth.py)
                ...
                # Print servo angles to console - only for the first block size
                ...
                # Check for quit (non-blocking check)
                # Note: For headless systems, you might want to use a different method
                try:
                    ...
                    # If display is not available, just continue
                    pass
...
if __name__ == "__main__":
    ...
    if ARDUCAM_AVAILABLE:
        ...
        processor = VideoProcessor(num_blocks=[3, 30], video_source=0, use_arducam=True)
    else:
        print("ArduCAM SDK not available, using OpenCV VideoCapture")
        # For webcam (fallback) - can also use multiple block sizes
        processor = VideoProcessor(num_blocks=[3, 30], video_source=0, use_arducam=False)
